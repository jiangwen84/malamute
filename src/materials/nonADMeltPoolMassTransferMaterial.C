...
#include "nonADMeltPoolMassTransferMaterial.h"

registerADMooseObject("MalamuteApp", nonADMeltPoolMassTransferMaterial);

InputParameters
nonADMeltPoolMassTransferMaterial::validParams()
{
...
nonADMeltPoolMassTransferMaterial::nonADMeltPoolMassTransferMaterial(
    const InputParameters & parameters)
  : Material(parameters),
    _temp(coupledValue("temperature")),
    _melt_pool_mass_rate(declareProperty<Real>("melt_pool_mass_rate")),
    _melt_pool_mass_rate_dT(declareProperty<Real>("melt_pool_mass_rate_dT")),
    ...
    _saturated_vapor_pressure(declareProperty<Real>("saturated_vapor_pressure")),
    _saturated_vapor_pressure_dT(declareProperty<Real>("saturated_vapor_pressure_dT"))
{
}

void
nonADMeltPoolMassTransferMaterial::computeQpProperties()
{
  ...
  _saturated_vapor_pressure_dT[_qp] = _p0 *
  ...
  _melt_pool_mass_rate_dT[_qp] =
      std::sqrt(_m / (2 * libMesh::pi * _boltzmann)) * (1 - _beta_r) *
      (_saturated_vapor_pressure_dT[_qp] / std::sqrt(_temp[_qp]) -
