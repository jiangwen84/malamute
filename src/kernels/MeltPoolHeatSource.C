...
MeltPoolHeatSource::MeltPoolHeatSource(const InputParameters & parameters)
  : ADKernelValue(parameters),
    ...
    _power_function(getFunction("laser_power")),
...
MeltPoolHeatSource::precomputeQpResidual()
{
  ...
  ADReal r = std::abs(_ad_q_point[_qp](0) - laser_location(0));

  ADReal laser_source = 2 * _power_function.value(_t, p) * _alpha /
                        (libMesh::pi * Utility::pow<2>(_Rb)) *
