...
LevelSetCurvatureRegularization::validParams()
{
  ...
  params.addRequiredCoupledVar("level_set", "Vector variable of level set's regularized gradient.");
...
{
  const auto grad_c_norm = (_grad_c[_qp] + RealVectorValue(libMesh::TOLERANCE)).norm();
  ADRealVectorValue n = (_grad_c[_qp]) / grad_c_norm;
  return _test[_i][_qp] * (_u[_qp] + Real(libMesh::TOLERANCE)) -
         _grad_test[_i][_qp] *
             (n - _varepsilon * (_grad_u[_qp] + RealVectorValue(libMesh::TOLERANCE)));
}
